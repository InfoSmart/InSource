		...
		if (m_selectedArea)
		{
			...
			if (m_showAreaInfoTimer.HasStarted() && !m_showAreaInfoTimer.IsElapsed() )
			{
				...
				else
				{
					...
					if ( attributes & (NavAttributeType)NAV_MESH_DONT_SPAWN )		Q_strncat( attrib, "DONT_SPAWN ", sizeof( attrib ), -1 );
					if ( attributes & (NavAttributeType)NAV_MESH_PLAYER_START )		Q_strncat( attrib, "PLAYER_START ", sizeof( attrib ), -1 );
					if ( attributes & (NavAttributeType)NAV_MESH_OBSCURED )		Q_strncat( attrib, "OBSCURED ", sizeof( attrib ), -1 );
					if ( attributes & (NavAttributeType)NAV_MESH_SPAWN_AMBIENT )		Q_strncat( attrib, "SPAWN_AMBIENT ", sizeof( attrib ), -1 );
					if ( attributes & (NavAttributeType)NAV_MESH_SPAWN_HERE )		Q_strncat( attrib, "SPAWN_HERE ", sizeof( attrib ), -1 );

					if ( m_selectedArea->IsBlocked( TEAM_ANY ) ) Q_strncat( attrib, "BLOCKED ", sizeof( attrib ), -1 );

					#ifdef APOCALYPSE
						if ( m_selectedArea->IsBlocked( TEAM_INFECTED ) ) Q_strncat( attrib, "BLOCKED INFECTED ", sizeof( attrib ), -1 );
						if ( m_selectedArea->IsBlocked( TEAM_HUMANS ) ) Q_strncat( attrib, "BLOCKED HUMANS ", sizeof( attrib ), -1 );
						if ( m_selectedArea->IsBlocked( TEAM_SOLDIERS ) ) Q_strncat( attrib, "BLOCKED SOLDIERS ", sizeof( attrib ), -1 );
					#endif
