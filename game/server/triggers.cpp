...
void CChangeLevel::Activate( void )
{
	...
	m_iLastTrigger = gpGlobals->curtime;
}
...
void CChangeLevel::ChangeLevelNow( CBaseEntity *pActivator )
{
	...
	if ( gpGlobals->curtime < (m_iLastTrigger+1) )
		return;
	...
	// Verificamos que todos los jugadores esten dentro
	for ( int i = 1; i <= gpGlobals->maxClients; ++i )
	{
		...
		if ( transitionState == TRANSITION_VOLUME_SCREENED_OUT )
		{
			DevMsg( "Player %s isn't in the transition volume %s, aborting\n", pPlayer->GetPlayerName(), m_szLandmarkName );
			return;
		}
		...
		if ( transitionState == TRANSITION_VOLUME_NOT_FOUND )
		{
			...
			if ( pPlayer )
			{
				...
				if ( !playerInPVS )
				{
					Warning( "Player %s isn't in the landmark's (%s) PVS, aborting\n", pPlayer->GetPlayerName(), m_szLandmarkName );
					return;
				}
			}
		}
